...
//}
//var params = [window.location.href, JSON.stringify(errorMsg), JSON.stringify(url), lineNumber, colNumber, user, +new Date].join('||');
//
//var img = new Image;
//img.onload = img.onerror = function(){img = null;};
//img.src= 'https://jstracker.friendwell.com/jstracker.png?info=' + params;
//};
...
;(function($) {
	...
	var getSonMenus = function(menuConfigs, topMenuId) {
		return _.orderBy(_.filter(menuConfigs, {
			isDir: false,
			dir_id: topMenuId
		}), ["order"], ["asc"]);
	};
//	var getSecondLevelMenu = function(menuConfigs, topMenuId) {
//		return _.orderBy(_.filter(menuConfigs, {
//			isDir: false,
//			secondLevel: topMenuId
//		}), ["order"], ["asc"]);
//	};
	...
	var init = function(options, callback) {
		...
		callerWant = $.extend({}, {
			...
			aclFree: false,
			userToken: function() {
				return JSON.parse(localStorage.getItem("employee_current")).authKey;
			},
			restApi: function() {
			...
			fetch: function() {
				return $.ajax({
					...
					method: "GET",
					headers: {
						"x-auth-token": " " + callerWant.userToken()
					},
					dataType: "json",
					crossDomain: true,
					timeout: 5000,
		...
		//处理finalConfig
		//检查localStorage中是否已经存在finalConfig(如果开启debug,就讲finalConfig设置为null,目的是重新获取)
		finalConfig = callerWant.debug ? null : JSON.parse(localStorage.getItem("employee_finalConfig"));

		//检查localStorage中是否已经存在menuHtml(如果开启debug,就讲menuHtml设置为null,目的是重新获取)
		var menuHtml = callerWant.debug ? null : localStorage.getItem("employee_menuHtml");
		if(menuHtml !== null) {
			//直接使用缓存数据
			...
			if(finalConfig !== null) { //直接使用缓存数据
				...
				menuHtml = callerWant.handleHtml(_.orderBy(_.filter(_.clone(finalConfig), {
					isDir: true
				}), ["order"], ["asc"]), _.curry(getSonMenus)(_.clone(finalConfig)));
				localStorage.setItem("employee_menuHtml", menuHtml); //缓存生成的menu html string
				...
				if(callerWant.aclFree) {
					//缓存得到的用户权限菜单配置
					localStorage.setItem("employee_finalConfig", JSON.stringify(finalConfig));
					//使用调用者提供的生成菜单html函数来生成menu html string
					menuHtml = callerWant.handleHtml(_.orderBy(_.filter(_.clone(finalConfig), {
						isDir: true
					}), ["order"], ["asc"]), _.curry(getSonMenus)(_.clone(finalConfig)));
				...
				callerWant.fetch().done(function(data) {
					...
					callerWant.handleError(xhr, status, error);
				});
			}
		}
	...
	$.fwMenu.prototype.getLinkById = function(id) {
		...
		return _.clone(_.filter(finalConfig, {
			link_id: id
		})[0]);
	};
	...
	$.fwMenu.prototype.rebuildMenu = function(handle) {
		...
		var menuHtml = handle(_.orderBy(_.filter(_.clone(finalConfig), {
			isDir: true
		}), ["order"], ["asc"]), _.curry(getSonMenus)(_.clone(finalConfig)));
		localStorage.setItem("employee_menuHtml", menuHtml);
		callerWant.node.html(menuHtml);
	};
